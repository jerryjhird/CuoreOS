...
static bool heap_grow(size_t size_needed) {
	...
	if (pages < PAGES_PER_GROWTH)
		pages = PAGES_PER_GROWTH;
...
retry:;
	...
	while (curr) {
		...
		curr = curr->next;
		if (curr == NULL && !wrapped) {
			curr = head;
			wrapped = true;
		}
		if (curr == start && wrapped)
			break;
	}
...
void *realloc(void *ptr, size_t new_size) {
	...
	if (curr->magic != HEAP_MAGIC)
		panic("MEMORY CORRUPTION", "Realloc on invalid pointer!");
	...
	if (curr->next && curr->next->is_free && !curr->next->is_pool_block &&
		...
		if (global_kernel_config.debug == 1)
			logbuf_vwrite(LOG_LEVEL_DEBUG, "[REALLOC] In-place expansion\n");
	...
	if (global_kernel_config.debug == 1)
		logbuf_vwrite(LOG_LEVEL_DEBUG, "[REALLOC] Heap migration required\n");
...
void dump_memory_stats(void) {
	...
	logbuf_write("\n[HEAP] Total Managed:  ");
	logbuf_puthex(total_managed);
	logbuf_putc('\n');
	logbuf_write("[HEAP] User Allocated: ");
	logbuf_puthex(total_user_data);
	logbuf_putc('\n');
	logbuf_write("[HEAP] Free Available: ");
	logbuf_puthex(total_free_memory);
	logbuf_putc('\n');
	logbuf_write("[HEAP] Bookkeeping:    ");
	logbuf_puthex(total_metadata);
	logbuf_write("\n");
	...
	logbuf_vwrite(LOG_LEVEL_DEBUG, "[PMA]  Total pages:    ");
	logbuf_vputhex(LOG_LEVEL_DEBUG, total_p);
	logbuf_vwrite(LOG_LEVEL_DEBUG, "\n");
	logbuf_vwrite(LOG_LEVEL_DEBUG, "[PMA]  Free pages:     ");
	logbuf_vputhex(LOG_LEVEL_DEBUG, free_p);
	logbuf_vwrite(LOG_LEVEL_DEBUG, "\n");
	logbuf_vwrite(LOG_LEVEL_DEBUG, "[PMA]  Used pages:     ");
	logbuf_vputhex(LOG_LEVEL_DEBUG, used_p);
	logbuf_vwrite(LOG_LEVEL_DEBUG, "\n");
}
